...
public class Pursue extends Behavior {
    private static Logger logger = Logger.getLogger( Pursue.class );
	...
	public Pursue(PhysicsObject obj) {
		super(obj);
		
		_seek = new Seek(obj, new Vec2(0,0));
	...
	public SteeringOutput getSteering(float moveModifier, float turnModifier) {
		...
		Vec2 direction = tPos.sub(_entity.getPPosition());
		...
		_align.setTarget(tPos);
		
		SteeringOutput so = _seek.getSteering(moveModifier, turnModifier);
		if (so != null) { 
